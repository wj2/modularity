...
class FigureGeometry(ModularizerFigure):
    def __init__(self, fig_key="geometry_figure", colors=colors, **kwargs):
        fsize = (7, 5)
        cf = u.ConfigParserColor()
        cf.read(config_path)
    ...
    def make_gss(self):
        ...
        geom_grid = pu.make_mxn_gridspec(self.gs, 3, 2, 0, 100, 30, 70, 10, 10)
        vis_grid = pu.make_mxn_gridspec(self.gs, 3, 1, 0, 100, 0, 20, 10, 10)
        geom_axs = self.get_axs(geom_grid, sharey="all", sharex="row")
        vis_axs = self.get_axs(vis_grid, all_3d=True)
        gss["panel_vis_geom"] = np.concatenate((vis_axs, geom_axs), axis=1)

        specialization_grid = pu.make_mxn_gridspec(
            self.gs, 3, 1, 0, 100, 80, 100, 10, 10
        )
