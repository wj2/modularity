...
def create_parser():
    ...
    parser.add_argument(
        "--input_dim", default=20, type=int, help="dimensionality of input"
    )
    parser.add_argument(
        "--rep_dim",
        default=400,
    ...
    parser.add_argument(
        "--fdg_epochs", default=5, type=int, help="epochs to train DG for"
    )

    parser.add_argument("--group_size", default=5, type=int, help="size of groups")
    parser.add_argument(
        "--tasks_per_group", default=10, type=int, help="number of tasks for each group"
    )
    parser.add_argument("--n_groups", default=3, type=int, help="number of groups")
    parser.add_argument(
        "--model_type", default="linear", type=str, help="kind of model"
    )
    parser.add_argument(
        "--model_epochs", default=40, type=int, help="epochs to train model for"
    )
    parser.add_argument(
        "--model_batch_size",
        default=100,
    ...
    parser.add_argument(
        "--cumu_weight", default=0.5, type=float, help="default cumulative weight"
    )
    ...
    parser.add_argument("--activity_reg", default=0.01, type=float)
    parser.add_argument("--weight_init_const", default=None, type=float)
    parser.add_argument("--input_noise", default=0.01, type=float)
    ...
    parser.add_argument("--fdg_layers", nargs="+", default=(300,), type=int)
    parser.add_argument("--model_layers", nargs="+", default=(), type=int)
    parser.add_argument("--ccgp_n_train", default=2, type=int)
    parser.add_argument("--ccgp_fix_features", default=-1, type=int)
    parser.add_argument("--continuous_input", default=False, action="store_true")
    parser.add_argument("--twod_shape_input", default=False, action="store_true")
    img_net_address = (
        "https://tfhub.dev/google/imagenet/"
        "mobilenet_v3_small_100_224/feature_vector/5"
    )
    parser.add_argument("--image_pre_net", default=img_net_address, type=str)
    parser.add_argument("--discrete_mixed_input", default=False, action="store_true")
    parser.add_argument("--dm_input_mixing", default=0.5, type=float)
    parser.add_argument("--dm_input_mixing_denom", default=1, type=float)
    parser.add_argument("--remove_last_inp", default=False, action="store_true")
    ...
    parser.add_argument("--untrained_tasks", default=0.5, type=float)
...
if __name__ == "__main__":
    ...
    if args.discrete_mixed_input:
        ...
        fdg = dg.MixedDiscreteDataGenerator(
            inp_dim, n_units=args.rep_dim, mix_strength=mix_strength
        )
    elif args.twod_shape_input:
        twod_file = (
            "disentangled/datasets/" "dsprites_ndarray_co1sh3sc6or40x32y32_64x64.npz"
        )
        ...
        fdg.fit(epochs=args.fdg_epochs, verbose=False, batch_size=args.dg_batch_size)
    ...
    related_context_modularity = np.zeros(args.n_reps)
    related_context_subspace = np.zeros(args.n_reps)

    related_context_all_null = np.zeros((args.n_reps, args.model_epochs))
    rc_all_null_tasks = np.zeros((args.n_reps, args.model_epochs + 1, tasks_per_group))
    related_context_all = np.zeros_like(related_context_all_null)
    rc_all_tasks = np.zeros_like(rc_all_null_tasks)
    related_context_all_modularity = np.zeros_like(related_context_modularity)
    related_context_all_subspace = np.zeros_like(related_context_modularity)
    ...
    new_context_modularity = np.zeros_like(related_context_modularity)
    new_context_subspace = np.zeros_like(related_context_modularity)
    ...
    new_task_modularity = np.zeros_like(related_context_modularity)
    new_task_subspace = np.zeros_like(related_context_modularity)
    ...
    for i in range(args.n_reps):
        ...
        out_related = ma.new_related_context_training(
            fdg,
        ...
        hist = out_related["trained_hist"]
        hist_null = out_related["null_hist"]
        ...
        related_context_modularity[i] = out_related["initial_modularity"]
        related_context_subspace[i] = out_related["initial_subspace"]

        print("a")
        out_related_all = ma.new_related_context_training(
            fdg,
            model_type_str=model_type,
            **model_kwargs,
            **train_kwargs,
        )
        hist = out_related_all["trained_hist"]
        hist_null = out_related_all["null_hist"]
        ...
        related_context_all_modularity[i] = out_related_all["initial_modularity"]
        related_context_all_subspace[i] = out_related_all["initial_subspace"]

        print("b")
        out_new = ma.new_context_training(
            fdg,
            model_type_str=model_type,
            **model_kwargs,
            **train_kwargs,
        )
        hist = out_new["trained_hist"]
        hist_null = out_new["null_hist"]
        ...
        new_context_modularity[i] = out_new["initial_modularity"]
        new_context_subspace[i] = out_new["initial_subspace"]

        nt_model_kwargs = dict(**model_kwargs)
        nt_model_kwargs["n_tasks"] = nt_model_kwargs["n_tasks"] + args.novel_tasks
        print(nt_model_kwargs)
        out_new_task = ma.new_task_training(
            fdg,
            model_type_str=model_type,
        ...
        hist = out_new_task["trained_hist"]
        hist_null = out_new_task["null_hist"]
        ...
        #     pass
        zs[i] = np.stack((errs_ood.numpy(), errs_ind.numpy()), axis=0)
        # zs = None

    all_save = {
        "related context": (related_context_all, related_context_all_null),
        "related context geometry": (
            related_context_all_modularity,
            related_context_all_subspace,
        ),
        ...
        "new context geometry": (
            new_context_modularity,
            new_context_subspace,
        ),
    ...
    maux.save_model_information(None, args.output_folder, args=args, **all_save)
